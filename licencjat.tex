...
\item X = $\{$X$_{1}$, X$_{2}$, $\ldots$, X$_{m}\}$ - zbiór zmiennych lub inaczej cech opisujących obiekty, gdzie $m \in \mathbb{N}$, dodatkowo X$_{1}=\{x_{i1}\}$, co oznacza wartość 1-szej cechy dla $i$-tego obiektu, przy czym $i \in \mathbb{N}$
...
Dwie krawędzie $e$ i $f$ grafu $G$ są sąsiednie, jeśli mają wspólny wierzchołek.
$$
\begin{tikzpicture}[scale=.7]
  \node (v) at (-2,0) {$v$};
  \node (w) at (1,1) {$w$};
...
\begin{uwaga}
Digraf w porównaniu do grafu, składa się ze zbioru krawędzi będących uporządkowanymi parami wierzchołków, tj. elementów zbioru $V$. Mamy zatem, że $E \subset V \times V$, co można przyrównać do definicji relacji dwuargumentowej w zbiorze $V$, gdzie relacja=$E$=zbiór krawędzi, zaś dziedzina relacji=$V$=zbiór wierzchołków. W przypadku grafu, mamy że zbiór krawędzi jest zbiorem nieuporządkowanych par wierzchołków, tj. $E \subset \{\{a,b\}: a, b \in V \}$  
\end{uwaga}
...
\begin{tikzpicture}[scale=.7]
 ...
 \node (F) at (3,0) {$F$};
 \node (D) at (2,5) {$D$};
  \draw (C)--(A)--(B)--(D)--(C) --(E)-- (F); %--(e)--(d); -- (one) -- (d) -- (zero);
  \draw (A)--(F)--(E);
\end{tikzpicture}
$$
...
\newpage
\chapter{Zastosowanie wybranych metod porządkowania danych wielowymiarowych}
...
\section{Normalizacja zmiennych}
...
\end{itemize}
\begin{enumerate}
\item Celem standaryzacji jest otrzymanie zmiennych o odchyleniu standardowym równym 1 i wartości oczekiwanej  równej 0. W tym celu dla każdej zmiennej, będącej cechą obiektu oblicza się odchylenie standardowe na podstawie wartości tej zmiennej dla wszystkich obiektów, a także wartość oczekiwaną na tej samej zasadzie. W kolejnym kroku dla każdego obiektu liczymy jego znormalizowana wartość tj. od wartości zmiennej odejmujemy wartość oczekiwaną dla danej cechy, a otrzymaną różnicę dzielimy przez odchylenie standardowe dla tej cechy,co można to zapisać w postaci
$$
z_{ij}=\frac{x_{ij} - \Ex (x_j)}{\sigma(x_j)}, \quad i = 1,2, \ldots, n, \quad j=1,2,\ldots, m.
$$
gdzie
$z_{ij}$-wartość znormalizowanej zmiennej $j$-tej zmiennej, w $i$-tym obiekcie.


\item W pracy skorzystałam również z unitaryzacji, stosowanej w celu uzyskania zmiennych o ujednoliconym zakresie zmienności, u mnie przedział ten to $[0,1]$. W tym celu od wartości zmiennej w obiekcie, odejmuje się minimalną wartość występującej dla tej cechy, a następnie różnica ta dzielona jest przez różnicę między maksymalną a minimalną wartością zmiennej, poddanej normalizacji. Znormalizowana zmienna jest postaci
$$
z_{ij}=\frac{x_{ij} - \min\limits_{i}\{x_{ij}\}}{\max\limits_{i} \{x_{ij}\} - \min\limits_{i} \{x_{ij}\}}, \quad i = 1,2, \ldots, n, \quad j=1,2,\ldots, m. 
$$
\item Kolejna metoda normalizacyjna, wykorzystana w pracy to przekształcenie ilorazowe. Stosuje się  je w celu odniesieniu wartości zmiennej do ustalonej wartości - może to być wartość oczekiwana danej zmiennej na tle analizowanych obiektów, wartość minimalna lub maksymalna tej cechy. W pracy za tę wartość przyjęłam wartość oczekiwaną zmiennej. Każda wartość zmiennej dla danego obiektu jest dzielona przez wartość oczekiwaną tej zmiennej, a postać znormalizowanej zmiennej to
$$
z_{ij}=\frac{x_{ij}}{\Ex (x_j)}, \quad \Ex (x_j) \neq 0 \quad i = 1,2, \ldots, n, \quad j=1,2,\ldots, m.
$$ 
\item Przy zastosowaniu metody rang, wykorzystuje się normalizację rangową. Stosuje się ją przeważnie, gdy zmienne opisujące obiekty mierzone są na skali porządkowej. W pierwszym kroku wartości zmiennych opisujących obiekty zostają uporządkowane ze względu na ich wartości po procesie normalizacji. W kolejnym kroku wartościom zmiennej przyporządkowywane są rangi - czyli wartości liczbowe, będące najczęściej numerami miejsc zajmowanych przez obiekty w uporządkowanym zbiorze. Postać zmiennej znormalizowanej rangowo
$$
z_{ij}=h, \quad dla x_{hj}=x_{ij}, \quad i = 1,2, \ldots, n, \quad j=1,2,\ldots, m.
$$
gdzie 
$h$-ranga nadana $i$-temu obiektowi znajdującemu się na $h$-tym miejscu w uporządkowanym zbiorze, ze względu na wartość $j$-tej zmiennej. 
\end{enumerate}

\section{Stymulacja zmiennych}
W rozdziale 2 umieściłam sekcje dotyczącą użytych pojęć w pracy. Pojawiła się tam definicja stymulanty, destymulanty, nominanty. Pracując na danych niezbędne jest ujednolicenie ich charakteru. Najczęściej sprowadza się ono do zamiany destymulant i nominant na stymulanty. Tego typu transformacje nazywamy stymulacją. Można wyróżnić dwie najczęściej stosowane metody, tj. przekształcenie ilorazowe oraz przekształcenie różnicowe. W zależności od skali na której mierzone są zmienne, należy stosować odpowiednie przekształcenie stymulacyjne.

W przypadku przekształcenia ilorazowego można go stosować tylko dla zmiennych mierzonych na skali ilorazowej. Stymulacja destymulant wygląda nastepująco
$$
...
Dla zmiennych mierzonych na skali ilorazowej czy też przedziałowej. Stymulacja dla destumulant prezetuje się następująco
$$
