...
Zacznijmy jednak od implementacji metod porządkowania nieliniowego.
...
natomiast dla metody Hellwiga standaryzacji. Do powyższych
operacji, zostaną wykorzystane zbudowane ogólne funkcje
...
UWAGA - kolumny wymagające stymulacji, muszą zostać osobno poddane
działaniu poniższej funkcji, dodatkowo po każdym zastosowaniu funkcji,
należy nadpisać zbiór by zmiany zostały zapisane.
...
\NormalTok{funkcja_porzadkowanie_metoda_sum<-function(x)}
  \NormalTok{\{}
    \NormalTok{x[,}\StringTok{"zmienna_syntetyczna"}\NormalTok{] <-}\DecValTok{0} \CommentTok{#kolumna to zmienna_syntetyczna}
    \NormalTok{for(i in }\DecValTok{1}\NormalTok{:}\KeywordTok{nrow}\NormalTok{(x))}
    \NormalTok{\{}
      ...
      \NormalTok{\}}
    \NormalTok{x[i,}\KeywordTok{ncol}\NormalTok{(x)]=x[i,}\KeywordTok{ncol}\NormalTok{(x)]/(}\KeywordTok{ncol}\NormalTok{(x)-}\DecValTok{2}\NormalTok{)  }\CommentTok{#liczba kolumn - (kolumna_nr_indeksu+kolumna_zmienna_synt)}
    \NormalTok{\}}
...
\subsection{Porównanie wyników dla całego zbioru}
{
\newcommand{\VerbBar}{|}
\newcommand{\VERB}{\Verb[commandchars=\\\{\}]}
...
\subsection{Para 1: metoda rang i metoda
...
\subsection{Para 2: metoda rang i metoda
...
\subsection{Para 3: metoda sum i metoda
