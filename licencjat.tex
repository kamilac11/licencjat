...
Dla zmiennych mierzonych na skali ilorazowej czy też przedziałowej stosuje się przekształecenie różnicowe. Stymulacja dla destymulant prezentuje się następująco
$$
...
\section{Wybrane pojęcia z teorii mnogości, topologii i algebry liniowej}
...
\max\limits_{i} \{z_{ij}\} & \textrm{dla  } z_{j}^S,\quad j=1,2,\dots,m, \quad i=1,2,\dots,n, \\
\min\limits_{i} \{z_{ij}\} & \textrm{dla } z_{j}^D, \quad j=1,2,\dots,m, \quad i=1,2,\dots,n,
\end{array} \right. $$
...
\begin{equation}
s_{i}=\bigg(\frac{d_{i0}-\min\limits_{i}\{d_{i0}\}}{\max\limits_{i}\{d_{i0}\}-\min\limits_{i}\{d_{i0}\}} \bigg)^{p}, \quad i=1,2,\dots,m
\end{equation}
...
\begin{equation}
d_ii^{'}= \max\limits_{i^{'}} {d_{ii^{'}}}, \quad i,i^{'}=1,2,\dots,n, i\neq i^{'}.
\end{equation}
...
Punktem wyjściem metod aglomeracyjnych stanowi założenie, że każdy obiekt stanowi odrębną, jednoelementową grupę $(\mathrm{G_{r}}, r=1,2,\dots,z)$.
...
Na początku odległości między jednoelementowymi grupami obiektów $\mathrm{G_{1}},\dots,\mathrm{G_{z}}$ są elementami wyjściowej macierzy odległości $\bold{D}$. W macierzy $\bold{D}$ poszukiwane są najmniejsze odległości pomiędzy grupami obiektów:
\begin{equation}
d_{rr^{'}}= \min\limits_{ii^{'}} {d_{ii^{'}}}, \quad i=1,2,\dots,n_{r}, \quad i^{'}=1,2,\dots,n_{r^{'}}, \quad r,r^{'}=1,2,\dots,z, r\neq r^{'}.
\end{equation}
gdzie:
...
\end{equation}
\begin{center}
$i=1,2,\dots,n_{r}, \quad i^{'}=1,2,\dots,n_{r^{'}}, \quad r,r^{'}=1,2,\dots,z, \quad r \neq r^{'}, $
\end{center}
gdzie:

\begin{equation}
\bold{O_{i}}=[z_{ij}], \quad j=1,2,\dots,m.
\end{equation}
