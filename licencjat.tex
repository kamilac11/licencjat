...
Niech $Y$ będzie niepustym zbiorem, $\mathcal{F}$  $\sigma$-ciałem na $Y$ i $\overline{\mathbb{R}} = \mathbb{R} \cup \{-\infty, \infty \}$. Funkcję $f: Y \rightarrow \overline{\mathbb{R}}$ nazywamy mierzalną, jeżeli zbiór $\{ y \in Y: f(y) > a \}$ jest mierzalny przy dowolnym $a \in \mathbb{R}$.
\end{definition}
...
\begin{uwaga}
Średnią z próby oznaczamy $\overline{Y}$ i wyliczamy za pomocą wzoru: $\overline{Y}(\omega)=\frac{1}{n}\sum\limits_{i=1}^{n}{Y(\omega_{i})}$.
\end{uwaga}

\begin{uwaga}
Powyższa definicja dana jest w języku rachunku prawdopodobieństwa. W języku statystycznym próbę losową rozumiemy jako wartość pochodząca z realizacji pewnego doświadczenia.
\end{uwaga}
...
Przekształcenie ilorazowe można stosować tylko dla zmiennych mierzonych na skali ilorazowej. Stymulacja destymulant wygląda następująco:

%Niech $x^{D}$ oznacza macierz o charakterze destymulanty oraz $x^{N}$ analogicznie o charakterze nominanty.%
...
Niech dane będą dwa elementy $a$ i $b$. Parą uporządkowaną nazywamy parę postaci $<a,b>$, gdzie element $a$ jest poprzednikiem, zaś element $b$ jest następnikiem: $<a,b>=\{\{a\},\{a,b\}\}.$
\end{definition}
...
$v_{0}, v_{1}, \ldots, v_{m}$ są różne (z wyjątkiem, być może, równości $v_{0}=v_{m}$, gdzie $v_{0}$ to wierzchołek początkowy a $v_{m}$ to wierzchołek końcowy), nazywamy drogą. 
\end{definition}
...
\item jeśli dla każdego $a \in A$ istnieje taki element $x \in A$, że $ a\leq x$, to $x_0 \leq x$.
\end{enumerate}
\end{definition}
...
\item jeśli dla każdego $a \in A$ istnieje taki element $y \in A$, że $ y \leq a$, to $y \leq y_0$.
\end{enumerate}
\end{definition}
...
$d_{0}$ - wartość wyznaczana dla każdej zmiennej, będąca sumą średniej odległości od obiektu
 ...
 oraz podwojonej wartości odchylenia standardowego dla tej zmiennej
...
Końcowa postać zmiennej syntetycznej otrzymywana jest, przy wykorzystaniu normalizacji postaci: %po przeprowadzeniu normalizacji według wzoru:
$$
...
Metody porządkowania nieliniowego dzielimy na metody dendrytowe oraz aglomeracyjne. Metody dendrytowe prowadzą do powstania dendrytu, prezentującego położenie obiektów ze względu na ich podobieństwo między sobą. Z kolei metody aglomeracyjne sprowadzają się do powstania grafu będącym łańcuchem połączeń, prezentującym sposób łączenia obiektów do siebie podobnych. 
...
%02.07 
%tzn. każdemu obiektowi przyporządkowujemy obiekt najbliżej od niego położony, nie będący jednocześnie nim samym. W takim razie pierwszemu zostaniemy przyporządkowany obiekt 2, obiektowi 2 z kolei obiekt 3 itd. Wynik zaprezentujemy na dendrycie:
...
\item $\forall_{i\geq n} \quad \exists_{j,k \in \mathbb{N}} \quad C_{i}=C_{j} \cup C_{k}$
...
$$
d_{zz^{'}}= \mediana_{i,k} \{d_{ik}(\bf{O_i} \in G_{z}, O_{k} \in G_{z^{'}})\},
$$
...
Poniżej zostaną przedstawione metody stymulacji zmiennych opisane wcześniej w rozdziale \ref{wybrane_operacje_statystyczne}. Ograniczamy
się tu do przypadku, że dana zmienna jest destymulantą i należy
ją poddać stymulacji. W tym celu stworzyliśmy dwie funkcje:
stymulacja\_przeksztalcenie\_ilorazowe(x,y) oraz
stymulacja\_przeksztalcenie\_roznicowe(x,y). W miejscu argumentu x
...
stymulacji, z tym że nazwa kolumny musi zostać podana w ``''.
...
\subsection{Metody porządkowania liniowego}
{
...
%poniższą funkcję tj. funkcja\_porzadkowanie\_metoda\_sum, zwraca ona
%numery indeksów obiektów wg kolejności, która uzyskały one po
...
\NormalTok{funkcja_porzadkowanie_metoda_rang<-function(x)}
  ...
  \NormalTok{\}}
  \NormalTok{x<-x[}\KeywordTok{order}\NormalTok{(x$zmienna_syntetyczna),]}
  \KeywordTok{print}\NormalTok{(}\StringTok{"Numery indeksów obiektów po uporządkowaniu: "}\NormalTok{)}
  \KeywordTok{return}\NormalTok{(x[}\DecValTok{1}\NormalTok{])}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{funkcja_porzadkowanie_metoda_Hellwiga<-function(x)}
  ...
  \NormalTok{\}}
  \NormalTok{odleg[i,}\DecValTok{2}\NormalTok{]=}\KeywordTok{sqrt}\NormalTok{(SUMKA) }\CommentTok{#kolumna zawierajaca odleglosci}
  \NormalTok{\}  }
