...
\begin{proof}Poniżej zostaną udowodnione powyższe własności.
\begin{enumerate}
...
\item Niech $x,y \in X$ oraz niech $ySx$. Korzystając z definicji sąsiada \ref{def-sasiada} mamy, że skoro $ySx$ to  $\left(y \leq x \lor x\leq y \right)\quad \land \quad  \left(\lnot \exists_{z \in X}  \quad x\neq z \neq y \Rightarrow   y \leq z \leq x \lor x \leq z \leq y \right)$. Zatem, wobec spójności porządku liniowego z Definicji \ref{def-porzadek-liniowy}, zachodzi $$ y\leq x \lor x\leq y.$$ Dzięki, czemu widać, że elementy są porównywane oraz możemy zapisać, że $xSy$. Zatem ostatecznie $ySx \Rightarrow xSy$.
...
Ale z faktu, że $x_m$ jest elementem minimalnym wynika, że $x_m \leq x$, zatem $x$ nie może być $x \leq x_m$, stąd $x$ jest  jedynym sąsiadem $x_m$, stąd element ten, nie może mieć jedynie pojedynczego sąsiada, ze względu na istnieje jeszcze elementu maksymalnego, którym element $x$ nie jest.\\
...
\begin{example}
Rozważmy zbiór dwuelementowy $X = \{ a, b \}$ gdzie $a \leq b$ jedynym punktem tej relacji. Tak zdefiniowana relacja spełnia wszystkie własności, ale nie spełnia założenia o przechodniości - zatem relacja ta nie jest liniowym porządkiem.
\end{example}
...
Z diagramu widać, że taka relacja spełnia wszystkie omawiane wcześniej własności - jednak brakuje jej cech spójności. I tak np. nie możemy porównać elementów $a$ i $d$, bowiem nie możemy określić czy  $d \leq a$ lub $a \leq d$.
\end{example}
